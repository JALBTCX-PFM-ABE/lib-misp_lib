...
void min_curve (float *weighted_mean, int32_t *reg_quadrant, 
    float *x, float *y, float *z, float *sorted_pos, 
    int32_t *reg_output, int32_t reg_width, int32_t reg_height, 
    int32_t x_max_num, int32_t y_max_num, float x_increment,
    float y_increment, float delta, int32_t *error, 
    double reg_x_min, double reg_y_min, float x_max_dist, 
    float y_max_dist, int32_t num_points, int32_t reg_multfact, 
    float squares, float search_radius, float search_radius2, 
    float convergence)
{
    int32_t      index1, index2, mode, loop2, position1, x_pos, y_pos,
                 reg_width_mult, reg_height_mult, lreg_multfact, value1, 
                 value2, value3;
            
    double       weight_value1, weight_value2, small, xvalue, yvalue, xi, eta,
                 absxi, abseta, dist1, dist2, temp_weight;
    uint8_t      prog_reg = NVFalse;


    void misp_iterate (float *, int32_t *, int32_t, int32_t, float, 
                       float *, float *, float *, int32_t, double, 
                       double, double, double);
    float weight_mean (double, double, float, float, 
                            int32_t, float, float, double, double, int32_t, 
    ...
    while (1)
    {
        ...
        xvalue = (float) (1.0 / ((float) x_increment * (float) lreg_multfact));
        yvalue = (float) (1.0 / ((float) y_increment * (float) lreg_multfact));
        ...
        for (index1 = 0; index1 < num_points; index1++)
        {
            ...
            x_pos = (int32_t) (xi + 0.5);
            y_pos = (int32_t) (eta + 0.5);
            xi -= x_pos;
            eta -= y_pos;
