...
void interpolate (float interval, int32_t length_x, 
    float start_xinterp, float end_xinterp, int32_t *length_xinterp,
    float *x, float *y, float *x_interp, float *y_interp)
{
    int32_t    num_segments, count, length2, endloop, index, seg_points, flag1,
               flag2, last_x, first_x, spline_length, current_x, new_last_x,
               new_first_x;
            
    float ater, x_pos, y_pos;   

    void spline (float *, float *, int32_t, float, 
        float *, float *);
    ...
    *length_xinterp =
    ...
    while (!endloop)
    {
        ...
        length2 = fabs ((double) ((*(x + last_x) - start_xinterp) / 
            interval)) + 1.0;
