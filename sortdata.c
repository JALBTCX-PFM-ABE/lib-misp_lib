...
void sortdata (float *sorted_pos, float *x, float *y, 
    float *z, float *reg_data, float reg_x_max, 
    double reg_x_min, float reg_y_max, double reg_y_min, 
    int32_t x_max_num, int32_t y_max_num, float x_max_dist, 
    float y_max_dist, int32_t num_points, int32_t reg_grid)
{
    int32_t      position1, position2, position3, index1, index2, index3;
    float        tempval;      
    ...
    for (index1 = 0; index1 < x_max_num; index1++)
    {
        ...
        {
            if ((((int32_t) ((*(reg_data + index2) - reg_x_min) / 
        ...
        for (index2 = 0; index2 < y_max_num; index2++)
        {
            *(sorted_pos + (index1 * y_max_num) + index2) = position1;
            ...
            if (position1 <= position3)
            {
                ...
                {
                    if (((int32_t)((*(y + index3) - reg_y_min) / y_max_dist)
